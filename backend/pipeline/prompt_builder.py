...
- CRITICAL — qualify shared CTE columns: When two or more CTEs expose a column with the same name (e.g. multiple per-program CTEs each carrying `process_name`), every reference to that column in the outer SELECT, JOIN, GROUP BY, and ORDER BY MUST be qualified with the CTE alias (e.g. `cpu_usage.process_name`). Unqualified references fail with `column reference "X" is ambiguous`.
