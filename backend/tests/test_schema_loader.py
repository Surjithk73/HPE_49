...
def test_normal_operation():
    ...
    try:
        ...
        print("[OK] All tables have columns")
        ...
        print("[OK] Can access specific table data")
        ...
        print(f"[OK] Found {len(queryable)} queryable columns in CPU table")
        ...
        print("[OK] Column existence check works")
        ...
        print(f"[OK] Can get table description (length: {len(desc)} chars)")
...
def test_edge_cases():
    ...
    try:
        ...
        print("[OK] Correctly raises FileNotFoundError for missing file")
        passed += 1
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            print("[FAIL] Should have raised ValueError for empty file")
            failed += 1
        except ValueError as e:
            if "empty" in str(e).lower():
                print("[OK] Correctly raises ValueError for empty file")
                passed += 1
            else:
    ...
    try:
        ...
        try:
            loader = SchemaLoader(temp_file)
            print("[FAIL] Should have raised exception for invalid YAML")
            failed += 1
        except Exception:
            print("[OK] Correctly raises exception for invalid YAML")
            passed += 1
        finally:
            os.unlink(temp_file)
    except Exception as e:
    ...
    try:
        ...
        try:
            loader = SchemaLoader(temp_file)
            print("[FAIL] Should have raised ValueError for missing tables")
            failed += 1
        except ValueError as e:
            if "missing" in str(e).lower():
                print("[OK] Correctly raises ValueError for missing required tables")
                passed += 1
            else:
                print(f"[FAIL] Wrong error message: {e}")
                failed += 1
    ...
    try:
        ...
        try:
            loader = SchemaLoader(temp_file)
            print("[FAIL] Should have raised ValueError for table without columns")
            failed += 1
        except ValueError as e:
            if "columns" in str(e).lower():
                print("[OK] Correctly raises ValueError for table without columns")
                passed += 1
            else:
                print(f"[FAIL] Wrong error message: {e}")
                failed += 1
    ...
    try:
        ...
        print("[FAIL] Should have raised KeyError for non-existent table")
        failed += 1
    except KeyError:
        print("[OK] Correctly raises KeyError for non-existent table")
        passed += 1
    except Exception as e:
        print(f"[FAIL] Wrong exception type: {type(e).__name__}")
        failed += 1
    ...
    try:
        ...
        print("[OK] Returns False for column check in non-existent table")
        passed += 1
    except Exception as e:
        print(f"[FAIL] Unexpected error: {e}")
        failed += 1
...
def test_data_integrity():
    ...
    try:
        ...
        print("[OK] Checked common columns across tables")
        ...
        if isinstance(sample_col, dict):
            print("[OK] Column definitions are dictionaries")
        else:
        ...
        print(f"[OK] Queryable columns ({len(queryable_cols)}) ≤ All columns ({len(all_cols)})")
        ...
        print(f"[OK] {desc_count}/{len(loader.get_table_names())} tables have descriptions")
        ...
        print("[OK] Schema returns consistent data")
...
def test_performance():
    ...
    try:
        ...
        print(f"[OK] Schema loaded in {load_time*1000:.2f}ms")
        ...
        print(f"[OK] Average table access: {access_time*1000:.4f}ms")
        ...
        print(f"[OK] Average column lookup: {lookup_time*1000:.4f}ms")
