...
export default function Landing() {
  return (
    <div style={{ minHeight: '100vh', background: 'var(--theme-bg)', color: 'var(--theme-tx-primary)', fontFamily: 'JetBrains Mono, Fira Code, Consolas, monospace' }}>
      {/* Navbar */}
      <header style={{ borderBottom: '1px solid var(--theme-border)', background: 'rgba(17,17,17,0.8)', backdropFilter: 'blur(12px)', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '64px' }}>
          <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '12px', textDecoration: 'none' }}>
            ...
            <div>
              <div style={{ fontSize: '15px', fontWeight: 700, color: 'var(--theme-tx-primary)', letterSpacing: '-0.02em' }}>QueryCraft</div>
            </div>
          </Link>
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
            <Link to="/how-it-works" style={{ color: 'var(--theme-tx-muted)', fontSize: '13px', textDecoration: 'none', fontWeight: 500, transition: 'color 0.2s' }} onMouseEnter={e => e.currentTarget.style.color = 'var(--theme-tx-primary)'} onMouseLeave={e => e.currentTarget.style.color = 'var(--theme-tx-muted)'}>
              How it Works
            </Link>
            <Link to="/dashboard" className="hpe-gradient-btn" style={{ padding: '8px 16px', borderRadius: '8px', background: 'var(--theme-accent)', color: 'var(--theme-tx-primary)', fontSize: '13px', textDecoration: 'none', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = 'var(--theme-accent-dim)'} onMouseLeave={e => e.currentTarget.style.background = 'var(--theme-accent)'}>
      ...
      {/* Hero Section */}
      <main>
        <section style={{ padding: '120px 24px', textAlign: 'center', background: 'radial-gradient(circle at 50% 0%, var(--theme-blue-tint) 0%, transparent 50%)' }}>
          <div style={{ maxWidth: '800px', margin: '0 auto' }}>
            ...
            <p style={{ fontSize: '18px', color: 'var(--theme-tx-secondary)', lineHeight: 1.6, marginBottom: '48px', maxWidth: '600px', marginInline: 'auto' }}>
        ...
        {/* Value Props */}
        <section style={{ padding: '80px 24px', borderTop: '1px solid var(--theme-border)', background: 'var(--theme-bg)' }}>
          <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            <div style={{ textAlign: 'center', marginBottom: '64px' }}>
              ...
              <p style={{ color: 'var(--theme-tx-secondary)', fontSize: '16px' }}>QueryCraft bypasses the need for manual SQL composition, giving you immediate insights.</p>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
              {[
                { icon: MessageSquare, color: 'var(--theme-accent)', title: 'Natural Language to SQL', desc: 'Ask questions in plain English. Our fine-tuned Gemini model generates complex JOINs and aggregations automatically.' },
                ...
                <div key={i} style={{ padding: '32px', background: 'var(--theme-surface-1)', border: '1px solid var(--theme-border)', borderRadius: '16px' }}>
                  ...
                  <h3 style={{ fontSize: '18px', fontWeight: 600, color: 'var(--theme-tx-primary)', margin: '0 0 12px' }}>{feature.title}</h3>
                  <p style={{ fontSize: '14px', color: 'var(--theme-tx-secondary)', lineHeight: 1.6, margin: 0 }}>{feature.desc}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Footer */}
      <footer style={{ borderTop: '1px solid var(--theme-border)', background: 'var(--theme-bg)', padding: '40px 24px', textAlign: 'center' }}>
